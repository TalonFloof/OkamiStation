...
MINIRV32_DECORATE int32_t MiniRV32IMAStep(struct MiniRV32IMAState* state,
...
MINIRV32_DECORATE int32_t MiniRV32IMAStep(struct MiniRV32IMAState* state,
  ...
  if ((CSR(timerh) > CSR(timermatchh) ||
    ...
    CSR(mip) |=
        1 << 7;  // MTIP of MIP // https://stackoverflow.com/a/61916199/2926815
                 // Fire interrupt.
  } else
    CSR(mip) &= ~(1 << 7);

  // If WFI, don't run processor.
  if (CSR(extraflags) & 4) return 1;

  int icount;

  for (icount = 0; icount < count; icount++) {
    uint32_t ir = 0;
    uint32_t trap = 0;  // If positive, is a trap or interrupt.  If negative, is
                        // fatal error.
    uint32_t rval = 0;
    ...
    if (ofs_pc & 3)
      ...
      switch (ir & 0x7f) {
        ...
        case 0b1101111:  // JAL
        {
          int32_t reladdy = ((ir & 0x80000000) >> 11) |
                            ((ir & 0x7fe00000) >> 20) |
        ...
        case 0b1100011:  // Branch
        {
          ...
          switch ((ir >> 12) & 0x7) {
            ...
            case 0b101:
              ...
              break;  // BGE
            case 0b110:
              if ((uint32_t)rs1 < (uint32_t)rs2) pc = immm4;
              break;  // BLTU
            case 0b111:
              if ((uint32_t)rs1 >= (uint32_t)rs2) pc = immm4;
              break;  // BGEU
        ...
        case 0b0110011:  // Op
        {
          ...
          if (is_reg && (ir & 0x02000000)) {
                                     ...
                                     // commands.  Be careful.
            {
              case 0b000:
                rval =
                    (is_reg && (ir & 0x40000000)) ? (rs1 - rs2) : (rs1 + rs2);
                break;
              case 0b001:
                rval = rs1 << rs2;
                break;
              ...
              case 0b101:
                rval =
                    (ir & 0x40000000) ? (((int32_t)rs1) >> rs2) : (rs1 >> rs2);
                break;
              case 0b110:
                rval = rs1 | rs2;
                break;
        ...
        case 0b0001111:
          rdid = 0;  // fencetype = (ir >> 12) & 0b111; We ignore fences in this
                     // impl.
          break;
        case 0b1110011:  // Zifencei+Zicsr
        {
          ...
          } else if (microop == 0b000)  // "SYSTEM"
          {
            ...
            if (csrno == 0x105)  // WFI (Wait for interrupts)
            {
              ...
              CSR(extraflags) |= 4;  // Infor environment we want to go to
                                     // sleep.
              SETCSR(pc, pc + 4);
              return 1;
            } else if (((csrno & 0xff) == 0x02))  // MRET
            {
              ...
              SETCSR(mstatus, ((startmstatus & 0x80) >> 4) |
    ...
    if (trap) {
      ...
      if (!(trap & 0x80000000)) CSR(extraflags) |= 3;
    }
