...
int KarasuWrite(uint32_t addr, uint32_t len, void *buf) {
    if (addr < 0x1000) { // Registers
        if(addr == 0x0) {
            ...
            memcpy((void*)&KarasuPalette,(const void*)&DefaultPal,sizeof(KarasuPalette));
        ...
        if(FBMode == 1) {
            ...
            uint32_t x = (addr/2)%800;
		    uint32_t y = (addr/2)/800;
            uint32_t x1 = ((addr/2)+(len/2)-1)%800;
		    uint32_t y1 = ((addr/2)+(len/2)-1)/800;
            MarkDirty(x,y,x1,y1);
        } else {
            MarkDirty(0,0,0,0);
        }
...
void KarasuInit() {
    ...
    memset((void*)framebuffer,0,1*1024*1024);
...
void KarasuUploadFrame() {
    ...
    if(FBMode != 0) {
        rect.x = DirtyX1;
		rect.y = DirtyY1;
		rect.w = DirtyX2-DirtyX1+1;
		rect.h = DirtyY2-DirtyY1+1;
    }
    if(FBMode == 0) {
        uint32_t* tex;
        int pitch;
        if(SDL_LockTexture(FBTexture,&rect,(void**)&tex,&pitch) != 0) {
            fprintf(stderr, "Texture Lock Error: %s\n", SDL_GetError());
            abort();
        }
        for(int y=0; y < 768; y++) {
            for(int x=0;x < 1024; x++) {
                tex[(y*1024)+x] = KarasuPalette[(framebuffer[(y*128)+(x/8)] & (1<<(7-(x%8)))) ? 255 : 0];
            }
        }
        ...
        // G: 5-9
        // B: 0-4
        uint64_t dirty_index = (DirtyY1*(800*2))+DirtyX1;
    }
    DirtyX1 = -1;
	DirtyX2 = 0;
	DirtyY1 = -1;
